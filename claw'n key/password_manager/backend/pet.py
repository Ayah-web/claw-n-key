"""
pet.py
Virtual pet state: hunger, happiness, energy.
...
# Mood thresholds (based on average of all stats)
def get_mood(avg):
    if avg >= 75:
        return "happy"
    elif avg >= 50:
...
class PetState:
    ...
    def _load(self):
        try:
            if os.path.exists(_PET_FILE):
                ...
                # No save file exists = definitely first launch
                self.is_first_launch = True
    ...
    def award_points(self, points, reason=""):
        ...
        # Adding passwords also slightly boosts happiness
        self.happiness = min(100.0, self.happiness + points * 0.5)
        self.save()
        print(f"[Pet] +{points} points ({reason}). Total: {self.points}")
